"""
Ingesta de salinidad — PredictaMAR Litoral (Pucusana)

FUENTE OPERATIVA: cmems_mod_glo_phy-so_anfc_0.083deg_PT6H-i
(CMEMS GLOBAL_ANALYSISFORECAST_PHY_001_024, versión 202406, parte default),
variable `so` (standard_name: sea_water_salinity), campos 3D INSTANTÁNEOS
cada 6 horas.

UNIDAD: la unidad nativa declarada por el producto es `1e-3`. Se conserva
tal cual y se transporta en cada lectura. NO se denomina "PSU" ni se
convierte: cualquier conversión exigiría una justificación científica
explícita que hoy no existe. La literatura pesquera suele reportar rangos
de salinidad en PSU; contrastar esos rangos con estos valores es una tarea
del análisis posterior, no de este módulo, que solo mide.

Verificación oficial del producto (ago 2026):
  - dataset_id confirmado en el catálogo, activo (retired: None)
  - resolución: paso ~0.0833° en latitud y longitud
  - cadencia: paso temporal de 21600000 ms = 6 horas
  - 50 niveles verticales
...
conserva ese cruce de fecha.

ALCANCE DE ESTA FUENTE:
Es SALINIDAD REGIONAL DE REFERENCIA, no una medición puntual ni in situ en
Pucusana. La grilla (~9 km) no resuelve la costa: la celda utilizada puede
estar a varios km del punto solicitado. Por eso cada SalinityReading lleva
su propio `data_scope` y `scope_warning`: la limitación viaja con el dato,
no solo en este docstring.

ESTRATEGIA DE PROFUNDIDAD:
El primer nivel vertical real del producto no es 0.0 m, y pedir
minimum_depth=0.0 genera una advertencia de rango excedido en cada
...
dataset devuelva -- así no se hardcodea ningún valor de profundidad (podría
cambiar en una versión futura del producto) ni se descargan los 50 niveles.
...
NUNCA se interpola, ni se sustituye, ni se fabrica un valor, ni se rellena
con un promedio. La excepción que provoque un SIN_DATOS se registra por
logging (sin credenciales), para que un error de programación no quede
indistinguible de una ausencia legítima de datos.

SIN kill switch: la salinidad es una variable candidata del futuro motor de
puntaje pesquero, no una compuerta de seguridad. A la fecha NO está
admitida al scoring (ver config/variables_spec.yaml). Aun así, SIN_DATOS
jamás debe convertirse en un valor favorable inventado.

Este módulo sigue deliberadamente el patrón ya auditado de
ingestion/fetch_temperature.py, por tratarse de la misma familia de
producto, la misma resolución y la misma cadencia.
"""
...
VARIABLE = "so"
UNITS = "1e-3"  # unidad nativa declarada por el producto; NO renombrar a PSU

TZ_PUCUSANA = ZoneInfo("America/Lima")  # UTC-5, sin horario de verano

DATA_SCOPE = "salinidad_regional_referencia"

DATA_SCOPE_WARNING = (
    "Salinidad regional de referencia, en la unidad nativa del producto "
    "(1e-3); no se convierte a otra escala. Cuando existe una lectura "
    "válida, procede de uno o más valores instantáneos (producto PT6H-i, "
    "cadencia de 6 h) del nivel superficial disponible del modelo, obtenidos "
    ...
    "tenga dato en el instante seleccionado. NO es una medición puntual ni "
    ...
    "interpola ni se fabrica una salinidad."
)

# ---- BANDA DE BÚSQUEDA SUPERFICIAL -----------------------------------
# Techo de la consulta; el nivel efectivo es el más somero que devuelva el
# dataset dentro de esta banda, elegido dinámicamente.
SURFACE_SEARCH_MAX_DEPTH_M = 5.0
# ----------------------------------------------------------------------

# ---- LÍMITE ESPACIAL (PROVISIONAL) -----------------------------------
# Perfil fetcher_puntual_phy_0083deg_pt6h: mitad de la diagonal de una
# celda de la grilla nativa (~9.2 km). Una celda a más de esa distancia ya
# está más cerca de la SIGUIENTE celda que del punto solicitado.
...
@dataclass
class SalinitySample:
    time_utc: datetime
    time_local: datetime
    value_salinity: float  # unidad nativa: ver UNITS
...
class SalinityReading:
    ...
    samples: list[SalinitySample] = field(default_factory=list)
    # ---- cobertura temporal ----
    ...
    # coverage_fraction: proporción de esos instantes nativos DENTRO de la
    ...
    depth_m_actual: float | None = None  # nivel real devuelto por el producto
    # ---- celda espacial ----
    ...
    status: SalinityStatus = SalinityStatus.SIN_DATOS
...
def _empty_reading(
    ...
    """
    Lectura SIN_DATOS: sin valores ni celda, pero con unidad, alcance y
    procedencia declarados igual que las demás. n_native_times_in_window se
    conserva cuando se conoce: permite distinguir "no había instantes
    nativos" de "los había pero ninguno tenía valor válido en una celda
    aceptable".
    """
    return SalinityReading(
        ...
        status=SalinityStatus.SIN_DATOS,
    )
...
def _cell_candidates(slab, lat: float, lon: float) -> list[tuple[float, float, float]]:
    ...
    No evalúa validez de valores aquí -- eso depende del/los instante(s).
...
def _value_at(slab, t_naive, cell_lat: float, cell_lon: float) -> float | None:
    """Valor nativo en un instante y celda concretos. None si es NaN."""
...
def fetch_salinity(
    ...
    """
    Devuelve las muestras instantáneas de salinidad del nivel superficial
    disponible, para la ventana horaria LOCAL de Pucusana solicitada.

    Interfaz pública única del módulo, conceptualmente equivalente a la de
    los demás fetchers puntuales del perfil PT6H: por defecto cubre el día
    local completo (00:00-23:59:59), que en Pucusana reúne los cuatro
    instantes nativos (01:00, 07:00, 13:00 y 19:00 local).

    Regla de selección espacial (una sola celda para toda la serie):
      1. Se evalúan las celdas usando los instantes DENTRO de la ventana.
    ...
    Fallback temporal (solo si no hay ningún valor válido en la ventana):
      5. Se consideran los instantes nativos fuera de la ventana hasta
      ...
      7. Nunca se interpola, sustituye ni inventan valores.
    ...
    try:
        ...
        # Clasificar instantes: dentro de la ventana, o fuera con su desfase
        ...
        for offset, _before_flag, t_utc, t_naive in out_window:
            for dist, cell_lat, cell_lon in candidates:  # ya ordenados por cercanía
                ...
                return SalinityReading(
                    ...
                    # internos solicitados: si había instantes nativos dentro
                    # de la ventana, todos quedaron sin cubrir.
                    ...
                    data_scope=DATA_SCOPE,
                    scope_warning=DATA_SCOPE_WARNING,
                    status=SalinityStatus.VALIDA_CERCANA_EN_TIEMPO,
                )
        ...
        logger.exception(
            "Fallo al obtener salinidad para (%s, %s) %s [%s]",
            lat, lon, target_date, DATASET_ID,
        )
        return _empty_reading(
            lat, lon, target_date, hour_start, hour_end,
